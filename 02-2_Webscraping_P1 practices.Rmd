...
```{r}
document1 <- read_html("https://en.wikipedia.org/wiki/Sola_Aoi")

#Let's take a look at the object we just created
document1


document2 <- read_html("https://en.wikipedia.org/wiki/Maria_Ozawa")
```
...
```{r}
artress <- html_nodes(document2, "#firstHeading") %>%
  html_text()


title <- html_nodes(document2, ".wikitable~ h3+ .wikitable td:nth-child(1)") %>% 
  html_text() #select text of element

title

company <- html_nodes(document2, ".wikitable~ h3+ .wikitable td:nth-child(2)") %>% 
  html_text() #select text of element

company


director <- html_nodes(document2, ".wikitable~ h3+ .wikitable td:nth-child(3)") %>% 
  html_text() #select text of element

director

release_date <- html_nodes(document2, ".wikitable~ h3+ .wikitable td:nth-child(4)") %>% 
  html_text() #select text of element

release_date

note <- html_nodes(document2, ".wikitable~ h3+ .wikitable td:nth-child(5)") %>% 
  html_text() #select text of element
...
df2 <- tibble(
  title, company, director, release_date, note)

df2 <- df2 %>%
  mutate(artress = artress)

df_tidy2 <- df2 %>%
  mutate(release_date =as_date( ifelse(is.na(mdy(release_date)),dmy(release_date),mdy(release_date))))



# verse <-verse_collapse %>% 
#   str_replace_all("[0-9]+", "@") %>%
#   as_tibble() %>%
